        ...
        member private x.selectElements() = 
            ...
            let rec selectElements' (acc:List<'n>) source =
                ...
                | Token.TagName(o, name) :: t -> 
                    ...
                    let selectedNodes = children |> Seq.filter(fun x -> navigator.GetName(x).Equals(name, StringComparison.OrdinalIgnoreCase)) |> Seq.toList
                ...
                | Token.OpenAttribute(o) :: Token.AttributeName(o1, name) :: Token.AttributeContainsWord(o2) :: Token.AttributeValue(o3, value) :: Token.CloseAttribute(o4) :: t ->
                    let selectedNodes = acc |> getTargets 
                                        |> Seq.filter (fun x -> 
                                                            ...
                                                            attr.Split(whiteSpaces).Any(fun s -> s.Equals(value, StringComparison.OrdinalIgnoreCase))                                                      )
                                        |> Seq.toList
                    level <- FilterLevel.Root
                    selectElements' selectedNodes t
                ...
                | Token.Checked(o) :: t ->
                    let selectedNodes = acc |> getTargets 
                                        |> Seq.filter (fun x -> (navigator.Attributes x).Allkeys |> List.contains "checked" )
                                        |> Seq.toList
                    level <- FilterLevel.Root
                    selectElements' selectedNodes t
                ...
                | Token.Selected(o) :: t ->
                    let selectedNodes = acc |> getTargets 
                                        |> Seq.filter (fun x -> (navigator.Attributes x).Allkeys |> List.contains "selected")
                                        |> Seq.toList
                    level <- FilterLevel.Root
                    selectElements' selectedNodes t
                ...
                | Token.Disabled(o) :: t ->
                    let selectedNodes = acc |> getTargets 
                                        |> Seq.filter (fun x -> (navigator.Attributes x).Allkeys |> List.contains "disabled")
                                        |> Seq.toList
                    level <- FilterLevel.Root
                    selectElements' selectedNodes t
                ...
                | Token.Enabled(o) :: t ->
                    let selectedNodes = acc |> getTargets 
                                        |> Seq.filter (fun x -> (navigator.Attributes x).Allkeys |> List.contains "disabled" = false)
                                        |> Seq.toList
                    level <- FilterLevel.Root
                    selectElements' selectedNodes t
